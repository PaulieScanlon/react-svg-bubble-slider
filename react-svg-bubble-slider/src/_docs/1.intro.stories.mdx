import { Meta } from '@storybook/addon-docs/blocks'

import { SvgBubbleSlider } from '../components/SvgBubbleSlider'
import { SvgIcon } from '../components/SvgIcon'

<Meta title="Intro" component={SvgBubbleSlider} />

<Box sx={{textAlign: 'left'}}>

<Flex>
  <Heading as="h1" sx={{ alignItems: 'center', flexGrow: 1 }}>
    SvgBubbleSlider
  </Heading>
  <Link
    href="https://github.com/PaulieScanlon/react-svg-bubble-slider"
    target="_blank"
    sx={{
      height: 24,
      color: 'text',
    }}
  >
    <ButtonIcon tabIndex={-1}>
      <svg
        xmlns="http://www.w3.org/2000/svg"
        role="img"
        viewBox="0 0 24 24"
        width="24"
        height="24"
        fill="currentColor"
      >
        <title>GitHub icon</title>
        <g>
          <path d="M12 .297c-6.63 0-12 5.373-12 12 0 5.303 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61C4.422 18.07 3.633 17.7 3.633 17.7c-1.087-.744.084-.729.084-.729 1.205.084 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 22.092 24 17.592 24 12.297c0-6.627-5.373-12-12-12" />
        </g>
      </svg>
    </ButtonIcon>
  </Link>
</Flex>

SvgBubbleSlider is fun way to add reactions to your blog posts 😃 😠 😥 and comes equipped with 14 built in reactions, but you don't have to use them all, see the [icons prop](/?path=/docs/svgbubbleslider--icons) to modify the icons list.

You can access the current [reaction](/?path=/docs/svgbubbleslider--reaction) via a render prop and use it to post to your back-end of choice.

Click or drag around to explore all the reactions

<Box
  sx={{
    '.svg-timeline': {
      ':focus': {
        outlineColor: 'accent',
      },
    },
    '.speech-bubble-text': {
      fill: 'primary',
      fontSize: '22px',
      textTransform: 'capitalize',
    },
  }}
>
  <SvgBubbleSlider>
    {({ reaction }) => (
      <Flex
        sx={{
          justifyContent: 'center',
          minHeight: 48,
          pt: 2,
        }}
      >
        {reaction && (
          <Button sx={{ textTransform: 'capitalize' }}>{reaction}</Button>
        )}
      </Flex>
    )}
  </SvgBubbleSlider>
</Box>

<Divider />

### Getting Started 🚀

You can install SvgBubbleSlider as a node module via `npm` or `yarn`

<Divider />

```sh
npm install react-svg-bubble-slider
```

### Usage 🧰

To use SvgBubbleSlider import it and add it to your React component's render method

```javascript
// some-component.js/.tsx
import React from 'react'

import { SvgBubbleSlider } from 'react-svg-bubble-slider'

const SomeComponent = () => {
  return (
    <div>
      ...
      <SvgBubbleSlider />
    </div>
  )
}

export default SomeComponent
```

<Divider />

### SvgIcon 😃 😠 😥

In addition to allowing users to leave a reaction you may wish to display how many reactions have been left. One way to do this is to use the `SvgIcon` and display the count underneath it.

<Box
  sx={{
    '.svg-timeline': {
      ':focus': {
        outlineColor: 'accent',
      },
    },
    '.speech-bubble-text': {
      fill: 'primary',
      fontSize: '22px',
      textTransform: 'capitalize',
    },
  }}
>
  <SvgBubbleSlider icons={['angry', 'smile', 'neutral']}>
    {({ reaction }) => (
      <Flex
        sx={{
          justifyContent: 'center',
          minHeight: 48,
          pt: 2,
        }}
      >
        {reaction && (
          <Button sx={{ textTransform: 'capitalize' }}>{reaction}</Button>
        )}
      </Flex>
    )}
  </SvgBubbleSlider>
</Box>

<Box sx={{ height: 20 }} />

<Flex
  sx={{
    justifyContent: 'space-around',
    margin: 'auto',
    flexWrap: 'wrap',
    maxWidth: 120,
    '.svg-icon': {
      color: 'accent',
    },
  }}
>
  <Flex sx={{ flexDirection: 'column' }}>
    <SvgIcon name="angry" />
    <Text as="small" variant="small">
      13
    </Text>
  </Flex>
  <Flex sx={{ flexDirection: 'column' }}>
    <SvgIcon name="smile" />
    <Text as="small" variant="small">
      22
    </Text>
  </Flex>
  <Flex sx={{ flexDirection: 'column' }}>
    <SvgIcon name="neutral" />
    <Text as="small" variant="small">
      1
    </Text>
  </Flex>
</Flex>

<Divider />

If you're using **react-svg-bubble-slider** in your project i'd love to hear from you [@pauliescanlon](https://twitter.com/PaulieScanlon)

[![ko-fi](https://www.ko-fi.com/img/githubbutton_sm.svg)](https://ko-fi.com/P5P31B7G8)

### Thanks 🙏

Thanks have to go to Chris Gannon for creating the original [SVG Bubble Slider](https://codepen.io/chrisgannon/pen/GZNgLw/)

### Licence ⚖️

GSAP and all GreenSock files are subject to the standard GreenSock license which can be found at [https://greensock.com/standard-license](https://greensock.com/standard-license)

</Box>
